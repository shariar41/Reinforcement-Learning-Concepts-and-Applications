...
#this function is used to capture easy temporal relationship by stacking n
#frames together
#for more complex temporal relationships 3D convolutions may be required
#following the DeepMind approach we stacked together the last 4 frames to
#produce a single image
#Each channel in the stacked image corresponds to a single frame at a
#different point in time,
#so the CNN can learn to extract features that represent the changes that
...
def Noop(env, actions_name, noop_max):
    ...
    for _ in range(noops):
        init_state, _, terminated, truncated, info = env.step(action)
...
#since we are using Pong without skipping frame integrated, we
#built a function to skip 4 frames at each step.
...
def skip_frames(action, env, skip_frame=4):
    ...
    for _ in range(skip_frame):
        ...
        done = truncated or terminated  # added
