...
def test(p_i, shared_model, params, max_steps, layers_):
    ...
    #initialize a queue for each env, preprocess each frame, obtain a vector of 84,84,4
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            action = probs.cpu().max(1, keepdim=True)[1].numpy()
        ...
        if done or (episode_length >= max_steps):
            ...
            frame_queue = initialize_queue(queue, layers_['n_frames'], in_state_i,
                                           env, actions_name)
            next_state = stack_frames(frame_queue)
            render = []
