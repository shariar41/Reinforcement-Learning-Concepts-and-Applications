...
def rollout(p_i, counter, params, model, hx, cx, frame_queue, env,
            current_state, episode_length, actions_name, layers_, tot_rew, scores, lock,
            avg_ep, scores_avg, iteration, device):
    #empty lists
    states = []
    actions = []
    rewards = []
    ...
    for _ in range(params['rollout_size']):
        ...
        masks.append(done)
        hx_s.append(hx.to(device))
        cx_s.append(cx.to(device))
        tot_rew += reward
        ...
        if done:
            ...
            frame_queue = initialize_queue(frame_queue, layers_['n_frames'],
                                           in_state_i, env, actions_name)
            ...
            # print('------------------------------------------------------')
            flag_finish, scores_avg = print_avg(scores, p_i, tot_rew, lock,
                                                avg_ep, params, flag_finish, scores_avg, iteration)
            #print('\n')
            ...
            tot_rew = 0
            hx = torch.zeros(1, layers_['lstm_dim']).to(device)
            cx = torch.zeros(1, layers_['lstm_dim']).to(device)
    ...
    return hx, cx, steps_array, episode_length, frame_queue, current_state, \
           tot_rew, counter, flag_finish, scores_avg


def compute_returns(steps_array, gamma, model, device):
    states, actions, rewards, masks, hx_s, cx_s, f_value = steps_array[0]
    model.train()  # added by AM
    R = f_value
    ...
    a = torch.tensor(actions).unsqueeze(1).to(device)
    hxs = torch.cat(hx_s)
    cxs = torch.cat(cx_s)
...
def ensure_shared_grads(local_model, shared_model):
    for param, shared_param in zip(local_model.parameters(), shared_model.parameters()):
        ...
        if shared_param.grad is not None:  # testing modified by AM
            return
        shared_param.grad = param.grad
        shared_param._grad = param.grad


def update_parameters(probs, log_probs, action_log_probs, advantages, returns,
                      values, value_coeff, entropy_coef):
    #policy loss
    policy_loss = -(action_log_probs.cpu() * advantages.detach()).mean()
...
def print_avg(scores, p_i, tot_rew, lock, avg_ep, params, flag_finish,
              array_avgs, iteration):
    with lock:
        ...
        if all_found == params['n_process']:
            ...
            if len(array_avgs) > 100:
                ...
                print('\n')
                print('------------------------------')
                print('AVG last 100 scores: ', avg, ' iteration=', iteration)
                print('------------------------------')
                print('\n')
