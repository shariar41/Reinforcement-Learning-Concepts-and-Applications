...
#pip install imageio
#pip install ale-py
#try without downgrading numpy, if it generates errors
#pip install numpy==1.23.5 numpy needs to be downgraded for atari gym

parser = argparse.ArgumentParser(description='A3C')
parser.add_argument('--lr', type=float, default=0.0001,  # was 0.0001
...
parser.add_argument('--n-workers', type=int, default=4,  # os.cpu_count(),
...
parser.add_argument('--env-name', default='PongNoFrameskip-v4',  # ALE/Pong-v5
...
#gym.pprint_registry() # print gym environments
device = 'cpu'  # important, will not train correctly on GPU because of multiple processes
...
if __name__ == '__main__':
    ...
    elif params['env_name'] == 'PongNoFrameskip-v4':
    ...
    if params['use_pre_trained'] == False:
        ...
        for p_i in range(0, n_processes):
            p = mp.Process(
                ...
                args=(p_i, shared_ac, params, optimizer, lock, counter_updates,
                      layers_, avg_ep, scores, scores_avg, flag_exit, device)
            )
            p.start()
            processes.append(p)
            time.sleep(5)
        ...
        if params['env_name'] == 'PongNoFrameskip-v4':
            ...
            trained = True
        elif params['env_name'] == 'BreakoutNoFrameskip-v5':
