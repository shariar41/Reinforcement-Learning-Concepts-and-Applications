...
def train(p_i, shared_model, p, optimizer, lock, counter, lys, avg_ep, scores,
          scores_avg, flag_exit, device):
    iteration = 0
    params = p.copy()
    layers_ = lys.copy()
    seed = params['seed']
    ...
    #initialize a queue for each env, preprocess each frame and obtain a vector of
    #84,84,4
    frame_queue = initialize_queue(queue, layers_['n_frames'], in_state_i, env,
                                   actions_name)
    ...
    while True:
        ...
        model.eval()  # added by AM

        #rollout_step
        hx, cx, steps_array, episode_length, frame_queue, current_state, tot_rew, counter, flag_finish, scores_avg = rollout(
            p_i, counter, params, model, hx, cx,
            frame_queue, env, current_state,
        ...
        #optimizer.zero_grad(set_to_none=False) # added by AM
        probs, log_probs, action_log_probs, advantages, returns, values = compute_returns(
            steps_array, params['gamma'], model, device
        )

        # compute losses and update parameters
        a3c_loss, value_loss, policy_loss, entropy_loss = update_parameters(
        ...
        if counter.value % 100 == 0:
            ...
            strout = f'Process: {p_i} Update: {counter.value} Policy_Loss: {policy_loss.item()} Value_Loss: {value_loss.item()} Entropy_Loss: {entropy_loss.item()} A3C loss: {a3c_loss.item()}'
            tqdm.write(strout)
            #pbar.write('----------------------------------------------------')
